...
function page_moyo($sql = '')
{
	...
	$flag = 'page';
		...
		$result = dbc(DBCMax)->query($sql_count)->limit(1)->done();
	$total = $result['MCNT'];
		if ($total <= $max)
	{
		return $sql;
...
function page_moyo_rewrite($action, $flag, $content)
{
	...
	if ($action == 'page.unset')
	{
		...
		if (!$rewriteHandler && (substr($url, -1) == '/' || substr($url, -4) == '.php')) $url .= '?show';
		return $url;
	}
	elseif ($action == 'page.rewrite')
	{
		...
		$prePageLNK = substr(page_moyo_request_uri(), -1) == '/' ? '' : '/';
		return preg_replace('/&'.$flag.'=(\d+)/i', $prePageLNK.$flag.'-$1', $content);
	}
}
...
function page_moyo_request_uri()
{
		$w2kURI = $_SERVER['HTTP_X_REWRITE_URL'];
	if ($w2kURI)
	{
		$url = $w2kURI;
	}
...
{
	$file404 = ini('settings.site_url').'/static/images/imager.404.png';
	if ($size > 0)
	{
		$width = $size;
	}
